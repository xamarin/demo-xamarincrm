...
namespace :testcloud do
	...
	task :ios => [:build_test_solution] do
		...
		sh "mono src/MobileApp/packages/Xamarin.UITest.1.0.0/tools/test-cloud.exe submit src/MobileApp/XamarinCRM.iOS/bin/iPhone/Debug/*.ipa #{API_KEY} --devices #{devices} --series \"#{DEVICE_SET}\" --locale #{LOCALE} --app-name \"Xamarin CRM\" --user #{USER_ACCOUNT} --assembly-dir src/MobileApp/XamarinCRM.UITest/bin/Debug --dsym src/MobileApp/XamarinCRM.iOS/bin/iPhone/Debug/*.app.dSYM"
	end

	desc "Push Android tests to XTC"
	task :android => [:build_test_solution] do
		devices = get_device_code(DEVICE_SET, "android")
		sh "mono src/MobileApp/packages/Xamarin.UITest.1.0.0/tools/test-cloud.exe submit src/MobileApp/XamarinCRM.Android/bin/Release/*Signed.apk #{API_KEY} keystore ~/.android/debug.keystore android androiddebugkey android --devices #{devices} --series \"#{DEVICE_SET}\" --locale #{LOCALE} --app-name \"Xamarin CRM\" --user #{USER_ACCOUNT} --assembly-dir src/MobileApp/XamarinCRM.UITest/bin/Debug"
	end
end
